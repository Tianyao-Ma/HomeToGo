package com.mty.hometogo.filter;
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        // filter by current token filter
        ...
        if (jwt != null && jwtUtil.validateToken(jwt)) {
            // if controller wants to use username, then their API just add parameter (principle) principle 可以通过 context holder match 出username;
            // the key is to add username to the spring security context holder.
        ...
        // after current filter, the following filter will continue to process;
